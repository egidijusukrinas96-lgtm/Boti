...
function portfolioMarkup() {
  return PORTFOLIO.map(a => `<div class="asset-row"><div><strong>${a.symbol}</strong><span>${a.network}</span></div><div class="asset-status">${a.mode}</div></div>`).join('');
}

function panelMarkup() {
  if (activePanel === 'deposit') return `<section class="card action-panel"><div class="panel-head"><div><div class="label">DEPOSIT · XRP</div><h2>Receive real XRP</h2></div><button id="closePanel" class="icon-btn">×</button></div><div class="asset-select"><button class="asset-choice active">XRP · XRPL MAINNET</button><button class="asset-choice disabled">BTC · COMING SOON</button><button class="asset-choice disabled">ETH · COMING SOON</button><button class="asset-choice disabled">USDT · COMING SOON</button></div>${wallet && !locked ? `<div class="deposit-box"><div class="label">SCAN OR COPY YOUR XRP ADDRESS</div><div class="qr-wrap"><canvas id="depositQr" width="220" height="220"></canvas></div><div class="deposit-address">${escapeHtml(wallet.address)}</div><button id="copyAddress">COPY XRP ADDRESS</button></div><div class="deposit-steps"><strong>HOW TO ADD MONEY</strong><ol><li>Copy the XRP address above or scan the QR.</li><li>Open your exchange/wallet and choose <b>XRP / XRP Ledger (XRPL)</b>.</li><li>Paste this address and send a small test amount first.</li><li>Return here and tap <b>REFRESH BALANCE</b>.</li></ol></div><button id="refreshDeposit" class="outline-btn">↻ REFRESH XRP BALANCE</button>` : '<div class="deposit-box"><div class="warning">Unlock your wallet to see the XRP address and QR code.</div><button id="unlockFromPanel">🔐 UNLOCK WALLET</button></div>'}<p class="hint">This is a real XRPL Mainnet deposit. The app does not take custody of your XRP. For a personal wallet, a destination tag is normally not needed; if your exchange specifically requires one for a withdrawal, follow that exchange's instructions and do not invent a tag.</p></section>`;
  if (activePanel === 'withdraw') return `<section class="card action-panel"><div class="panel-head"><div><div class="label">WITHDRAW · XRP</div><h2>Send real XRP</h2></div><button id="closePanel" class="icon-btn">×</button></div>${wallet && !locked ? `<input id="destination" placeholder="Destination r-address" autocomplete="off"/><input id="amount" inputmode="decimal" placeholder="Amount XRP"/><button id="send">🔐 PREVIEW · BIOMETRIC · SEND</button><p class="hint">You will see the exact amount, fee and destination before biometric authentication and local signing on XRPL Mainnet.</p>` : '<p class="warning">Unlock the wallet first to withdraw.</p><button id="unlockFromPanel">🔐 UNLOCK WALLET</button>'}</section>`;
  return '';
}

async function render() {
  const stored = await hasStoredWallet();
  app.innerHTML = `<main class="shell"><header class="topbar"><div><div class="eyebrow">ADRENALINE PROTOCOL</div><h1>ADRENALINE <span>WALLET</span></h1></div><div class="top-actions"><div class="status">● ${status}</div><button id="menuBtn" class="menu-btn" aria-label="Open menu">☰</button></div></header>${menuOpen ? `<nav class="menu card"><button id="menuPortfolio">◉ PORTFOLIO</button><button id="menuDeposit">↓ DEPOSIT</button><button id="menuWithdraw">↑ WITHDRAW</button><button id="menuWallet">▣ WALLET</button><button id="menuSecurity">⌕ SECURITY</button></nav>` : ''}<section class="card hero"><div class="label">UNIFIED PORTFOLIO · XRP MAINNET</div><div class="balance">${balance} <small>XRP</small></div><div class="sub">${wallet && !locked ? short(wallet.address) : stored ? 'Wallet locked' : 'No wallet configured'}</div>${wallet && !locked ? '<div class="unlock-state">🔓 WALLET UNLOCKED</div>' : '<div class="lock-state">🔒 WALLET LOCKED</div>'}<div class="quick-actions"><button id="depositBtn">↓ DEPOSIT XRP</button><button id="withdrawBtn" class="outline">↑ WITHDRAW XRP</button></div>${stored && (!wallet || locked) ? '<button id="unlock">🔐 UNLOCK WITH BIOMETRIC</button>' : ''}${!stored ? '<button id="create">CREATE NEW WALLET</button>' : ''}${!wallet || locked ? '<button id="import" class="ghost">IMPORT RECOVERY PHRASE</button>' : ''}${wallet && !locked ? '<button id="lock" class="ghost">LOCK WALLET</button>' : ''}</section>${panelMarkup()}<section id="portfolio" class="card portfolio"><div class="label">ASSET PORTFOLIO</div>${portfolioMarkup()}<p class="hint">XRP is live on XRPL Mainnet. BTC, ETH and ERC-20 USDT are not enabled yet.</p></section>${wallet && !locked ? `<section id="wallet" class="card wallet"><div class="label">XRPL MAINNET WALLET</div><div class="address">${escapeHtml(wallet.address)}</div><button id="refresh" class="ghost">REFRESH BALANCE</button></section><section id="security" class="card security"><div class="label">RECOVERY</div><p>Recovery phrase is kept only in the active app session and is never stored in secure storage.</p>${recoveryPhrase ? '<button id="showPhrase" class="ghost">SHOW RECOVERY PHRASE</button>' : '<p class="warning">Your recovery phrase is not available in this session. Use your original backup phrase if you need to restore the wallet.</p>'}<button id="delete" class="danger">DELETE DEVICE WALLET</button></section>` : `<section id="security" class="card security"><div class="label">SECURITY</div><p>Wallet secrets are stored with the native Android secure-storage plugin. Unlock requires biometric/device authentication.</p><p class="warning">Never share your recovery phrase. Anyone with it can control the wallet.</p></section>`}<section class="card protocol"><div class="label">GAME SAFETY</div><h2>USER CONTROLLED</h2><p>Victory/Death is a game layer. It never confiscates, burns or automatically transfers real wallet funds.</p></section><p id="message" class="message">${escapeHtml(message)}</p><footer>XRPL MAINNET · LOCAL SIGNING · USER CONTROLLED FUNDS</footer></main>`;
  ...
  document.querySelector('#refreshDeposit')?.addEventListener('click', refreshBalance);
  ...
  const qrCanvas = document.querySelector('#depositQr');
  if (qrCanvas && wallet && !locked) {
    try { await QRCode.toCanvas(qrCanvas, wallet.address, { width: 220, margin: 2, errorCorrectionLevel: 'M' }); } catch (e) { setMessage(`QR error: ${e.message}`, 'QR ERROR'); }
  }
}
...
async function createWallet() { try { const phrase = generateMnemonic(wordlist, 128); const nextWallet = makeWalletFromPhrase(phrase); await saveWallet(nextWallet); wallet = nextWallet; recoveryPhrase = phrase; locked = false; balance = await getBalance(); status = 'MAINNET WALLET CREATED'; message = 'Back up the recovery phrase now.'; await render(); setTimeout(() => showPhrase(true), 50); } catch (e) { setMessage(e.message, 'ERROR'); } }
async function importWallet() { try { const phrase = prompt('Enter your 12-word BIP39 recovery phrase. It stays on this device and is not uploaded.'); if (!phrase) return; const normalized = phrase.trim().toLowerCase().replace(/\s+/g, ' '); const nextWallet = makeWalletFromPhrase(normalized); await saveWallet(nextWallet); wallet = nextWallet; recoveryPhrase = normalized; locked = false; balance = await getBalance(); setMessage('Wallet imported. XRP Mainnet balance is now connected.', 'MAINNET READY'); } catch (e) { setMessage(e.message, 'IMPORT ERROR'); } }
async function unlock() { try { const stored = await unlockWallet(); wallet = Wallet.fromSeed(stored.seed); locked = false; recoveryPhrase = ''; balance = await getBalance(); activePanel = ''; setMessage('Wallet unlocked with biometric/device authentication.', 'MAINNET READY'); } catch (e) { setMessage(e.message, 'UNLOCK ERROR'); } }
function lockWallet() { wallet = null; recoveryPhrase = ''; balance = '0'; locked = true; activePanel = ''; setMessage('Wallet locked.', 'LOCKED'); }
async function showPhrase() { if (!wallet || locked || !recoveryPhrase) return; try { const stored = await unlockWallet(); const recovered = Wallet.fromSeed(stored.seed); if (recovered.address !== wallet.address) throw new Error('Wallet integrity check failed.'); alert(`RECOVERY PHRASE — KEEP SECRET\n\n${recoveryPhrase}\n\nNever share or screenshot this phrase.`); } catch (e) { setMessage(e.message, 'SECURITY ERROR'); } }
...
async function refreshBalance() { if (!wallet || locked) return setMessage('Unlock the wallet first.', 'LOCKED'); balance = await getBalance(); setMessage(`XRPL Mainnet balance: ${balance} XRP`, 'MAINNET READY'); }
...
async function sendXrp() {
  ...
  try {
    ...
    const ok = confirm(`REAL XRP TRANSACTION PREVIEW\n\nTo: ${destination}\nAmount: ${amount} XRP\nNetwork fee: ${fee} XRP\nSequence: ${prepared.Sequence}\nNetwork: XRPL MAINNET\n\nThis transaction uses real XRP. Continue to biometric authentication and local signing?`);
    ...
    balance = await getBalance(); activePanel = '';
    const txResult = result.result?.meta?.TransactionResult || result.result?.engine_result || '';
    if (txResult && txResult !== 'tesSUCCESS') throw new Error(`XRPL rejected transaction: ${txResult}`);
