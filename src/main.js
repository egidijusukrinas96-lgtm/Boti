...
function panelMarkup() {
  if (activePanel === 'deposit') return `<section class="card action-panel"><div class="panel-head"><div><div class="label">DEPOSIT · XRP</div><h2>Receive real XRP</h2></div><button id="closePanel" class="icon-btn">×</button></div><div class="asset-select"><button class="asset-choice active">XRP · XRPL MAINNET</button><button class="asset-choice disabled">BTC · COMING SOON</button><button class="asset-choice disabled">ETH · COMING SOON</button><button class="asset-choice disabled">USDT · COMING SOON</button></div><div class="deposit-box"><div class="label">YOUR XRP MAINNET ADDRESS</div><div class="deposit-address">${wallet && !locked ? escapeHtml(wallet.address) : 'Unlock wallet to view address'}</div><button id="copyAddress" ${wallet && !locked ? '' : 'disabled'}>COPY ADDRESS</button></div><p class="hint">Send only real XRP on the XRP Ledger Mainnet to this address. If an exchange asks for a destination tag, this personal wallet normally does not require one.</p></section>`;
...
async function refreshBalance() { if (!wallet || locked) return setMessage('Unlock the wallet first.', 'LOCKED'); balance = await getBalance(); setMessage('XRPL Mainnet balance refreshed.', 'MAINNET READY'); }
